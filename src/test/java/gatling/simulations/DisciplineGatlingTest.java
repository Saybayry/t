...
public class DisciplineGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all disciplines").get("/api/disciplines").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created discipline").get("${new_discipline_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created discipline").delete("${new_discipline_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
