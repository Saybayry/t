...
public class AssessmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all assessments").get("/api/assessments").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new assessment")
                        .post("/api/assessments")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"isPresent\": 0" + ", \"assessment\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created assessment").get("${new_assessment_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created assessment").delete("${new_assessment_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
