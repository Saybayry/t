 ...
 */
public class TeacherGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all teachers").get("/api/teachers").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new teacher")
                        .post("/api/teachers")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" + "\"fname\": \"SAMPLE_TEXT\"" + ", \"mname\": \"SAMPLE_TEXT\"" + ", \"lname\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created teacher").get("${new_teacher_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created teacher").delete("${new_teacher_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Teacher entity").exec(scn);
