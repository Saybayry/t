...
public class LessonGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all lessons").get("/api/lessons").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new lesson")
                        .post("/api/lessons")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"classDate\": \"2020-01-01T00:00:00.000Z\"" + ", \"classNumber\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created lesson").get("${new_lesson_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created lesson").delete("${new_lesson_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
